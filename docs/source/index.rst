...
:doc:`glint_classes` is the library of the pipeline containing the classes to deal with raw data and feed the model fitting algorithm.

:doc:`glint_functions` is the library of the pipeline containing the functions to deal with raw data and feed the model fitting algorithm.
...
:doc:`doc_nsc` performs the *Numerical Self-Calibration* (NSC).

:doc:`doc_nsc_lib` contains the functions to perform the NSC.

:doc:`doc_nsc_config` contains some settings for a given dataset to process.

The source can be found `on Github`_.

.. _on Github: https://github.com/SydneyAstrophotonicInstrumentationLab/BARNACLE
