...
from barnacle.glint_functions import getSpectralFluxDebug
import numpy as np
    ...
    def loadfile(self, data=None, nbimg=(0, 1)):
        ...
        if data != None:
            if '.mat' in data:
                ...
                with fits.open(data) as dataFile:
                    self.data = np.array(dataFile[0].data)
    ...
    def cosmeticsFrames(self, dark, nonoise=False):
        ...
        if nonoise:
            ...
            if not np.all(dark == 0):  # If 'dark' is not a 0-array
                ...
                self.data = self.data - \
                    self.data[:, :, :20].mean(axis=(1, 2))[:, None, None]
...
class Null(File):
    ...
    def getSpectralFlux(self, spectral_axis, positions, widths, mode_flux, debug=False):
        ...
        :Parameters:
            ...
            **debug**: bool
                Debug mode.
                If ``True``, use the python/numpy function ``getSpectralFluxDebug`` which is 
        ...
        if debug:
            ...
            self.raw_err = np.append(
                self.slices[:, :, :, :10-5], self.slices[:, :, :, 10+5:], axis=-1)
            self.raw_err = self.raw_err.std(
                axis=-1) / self.raw_err.shape[-1]**0.5
        ...
        # _getSpectralFlux(nbimg, which_tracks, slices_axes, slices, spectral_axis, positions, widths)
            self.amplitude_fit, self.amplitude, self.residuals_fit, self.residuals_reg, self.cov, self.amplitude_error = \
                getSpectralFluxDebug(nbimg, which_tracks, slices_axes,
                                     slices, spectral_axis, positions, widths)
        else:
            if mode_flux == 'raw':
                self.raw = self.slices[:, :, :, 10-4:10+5].mean(axis=-1)
                self.raw = np.transpose(self.raw, axes=(0, 2, 1))
                self.raw_err = self.slices[:, :, :, :10 -
                                           5].std(axis=-1) / slices_axes.shape[-1]**0.5
                self.raw_err = np.transpose(self.raw_err, axes=(0, 2, 1))
            else:
                try:
                    self.amplitude, self.residuals_reg, self.amplitude_error = self._getSpectralFluxNumba(
                        nbimg, which_tracks, slices_axes, slices, spectral_axis, positions, widths)
                except np.linalg.LinAlgError:
                    print('LinAlgError')
                    self.amplitude, self.residuals_reg, self.amplitude_error = self._getSpectralFluxNumba2(
                        nbimg, which_tracks, slices_axes, slices, spectral_axis, positions, widths)
    ...
    @staticmethod
    @jit(nopython=True)
    def _getSpectralFluxNumba(nbimg, which_tracks, slices_axes, slices,
                              spectral_axis, positions, widths):
        """
        Numba-ized function measuring the flux per spectral channel\
            (1 pixel width).
        ...
        for k in range(nbimg):
            for i in which_tracks:
                for j in range(len(spectral_axis)):
                    ...
                    residuals_reg[k, i, j] = slices[k, j, i] - \
                        (popt2[0] * simple_gaus +
                         popt2[1] + popt2[2] * slices_axes[i])
                    amplitude[k, i, j] = popt2[0]
                    chi2 = np.sum(
                        residuals_reg[k, i, j]**2) / (slices_axes[i].size-popt2.size)
                    error[k, i, j] = (
                        chi2 / np.sum((slices_axes[i] - np.mean(slices_axes[i]))**2))**0.5
    ...
    def getTotalFlux(self):
        ...
        self.fluxes = np.array(
            [self.fluxes[:, 15], self.fluxes[:, 13], self.fluxes[:, 2], self.fluxes[:, 0]])
    ...
    def matchSpectralChannels(self, wl_to_px_coeff, px_to_wl_coeff):
        ...
        # Tracks to process, former argument which functionality no longer exists
        which_tracks = np.arange(16)
    ...
    def error_null(self, null, Iminus, Iplus, Iminus_err, Iplus_err):
        ...
        """
        null_err2 = (Iminus_err**2 / Iminus**2 +
                     Iplus_err**2 / Iplus**2) * null**2
    ...
    def computeNullDepth(self):
        ...
        # Errors
        # With amplitude
        self.null1_err = self.error_null(
            self.null1, self.Iminus1, self.Iplus1, self.bg_std[:, None], self.bg_std[:, None])
        self.null2_err = self.error_null(
            self.null2, self.Iminus2, self.Iplus2, self.bg_std[:, None], self.bg_std[:, None])
        self.null3_err = self.error_null(
            self.null3, self.Iminus3, self.Iplus3, self.bg_std[:, None], self.bg_std[:, None])
        self.null4_err = self.error_null(
            self.null4, self.Iminus4, self.Iplus4, self.bg_std[:, None], self.bg_std[:, None])
        self.null5_err = self.error_null(
            self.null5, self.Iminus5, self.Iplus5, self.bg_std[:, None], self.bg_std[:, None])
        self.null6_err = self.error_null(
            self.null6, self.Iminus6, self.Iplus6, self.bg_std[:, None], self.bg_std[:, None])
    ...
    def getIntensities(self, mode, wl_bounds=(0, np.inf)):
        ...
        if mode == 'fit':
            ...
            self.Iminus1, self.Iplus1 = self.amplitude[:, 11][:,
                                                              self.px_scale[11]], self.amplitude[:, 9][:, self.px_scale[9]]
            self.Iminus2, self.Iplus2 = self.amplitude[:, 3][:,
                                                             self.px_scale[3]], self.amplitude[:, 12][:, self.px_scale[12]]
            self.Iminus3, self.Iplus3 = self.amplitude[:, 1][:,
                                                             self.px_scale[1]], self.amplitude[:, 14][:, self.px_scale[14]]
            self.Iminus4, self.Iplus4 = self.amplitude[:, 6][:,
                                                             self.px_scale[6]], self.amplitude[:, 4][:, self.px_scale[4]]
            self.Iminus5, self.Iplus5 = self.amplitude[:, 5][:,
                                                             self.px_scale[5]], self.amplitude[:, 7][:, self.px_scale[7]]
            self.Iminus6, self.Iplus6 = self.amplitude[:, 8][:,
                                                             self.px_scale[8]], self.amplitude[:, 10][:, self.px_scale[10]]
        ...
        #     self.p1_err = self.p2_err = self.p3_err = self.p4_err = self.windowed_err
            ...
            self.Iminus1, self.Iplus1 = self.raw[:, 11][:,
                                                        self.px_scale[11]], self.raw[:, 9][:, self.px_scale[9]]
            self.Iminus2, self.Iplus2 = self.raw[:, 3][:,
                                                       self.px_scale[3]], self.raw[:, 12][:, self.px_scale[12]]
            self.Iminus3, self.Iplus3 = self.raw[:, 1][:,
                                                       self.px_scale[1]], self.raw[:, 14][:, self.px_scale[14]]
            self.Iminus4, self.Iplus4 = self.raw[:, 6][:,
                                                       self.px_scale[6]], self.raw[:, 4][:, self.px_scale[4]]
            self.Iminus5, self.Iplus5 = self.raw[:, 5][:,
                                                       self.px_scale[5]], self.raw[:, 7][:, self.px_scale[7]]
            self.Iminus6, self.Iplus6 = self.raw[:, 8][:,
                                                       self.px_scale[8]], self.raw[:, 10][:, self.px_scale[10]]
        ...
        self.p1 = self.p1[:, (self.wl_scale[15] >= wl_bounds[0]) & (
            self.wl_scale[15] <= wl_bounds[1])]
        self.p2 = self.p2[:, (self.wl_scale[13] >= wl_bounds[0]) & (
            self.wl_scale[13] <= wl_bounds[1])]
        self.p3 = self.p3[:, (self.wl_scale[2] >= wl_bounds[0]) & (
            self.wl_scale[2] <= wl_bounds[1])]
        self.p4 = self.p4[:, (self.wl_scale[0] >= wl_bounds[0]) & (
            self.wl_scale[0] <= wl_bounds[1])]

        self.Iminus1 = self.Iminus1[:, (self.wl_scale[11] >= wl_bounds[0]) & (
            self.wl_scale[11] <= wl_bounds[1])]
        self.Iminus2 = self.Iminus2[:, (self.wl_scale[3] >= wl_bounds[0]) & (
            self.wl_scale[3] <= wl_bounds[1])]
        self.Iminus3 = self.Iminus3[:, (self.wl_scale[1] >= wl_bounds[0]) & (
            self.wl_scale[1] <= wl_bounds[1])]
        self.Iminus4 = self.Iminus4[:, (self.wl_scale[6] >= wl_bounds[0]) & (
            self.wl_scale[6] <= wl_bounds[1])]
        self.Iminus5 = self.Iminus5[:, (self.wl_scale[5] >= wl_bounds[0]) & (
            self.wl_scale[5] <= wl_bounds[1])]
        self.Iminus6 = self.Iminus6[:, (self.wl_scale[8] >= wl_bounds[0]) & (
            self.wl_scale[8] <= wl_bounds[1])]

        self.Iplus1 = self.Iplus1[:, (self.wl_scale[9] >= wl_bounds[0]) & (
            self.wl_scale[9] <= wl_bounds[1])]
        self.Iplus2 = self.Iplus2[:, (self.wl_scale[12] >= wl_bounds[0]) & (
            self.wl_scale[12] <= wl_bounds[1])]
        self.Iplus3 = self.Iplus3[:, (self.wl_scale[14] >= wl_bounds[0]) & (
            self.wl_scale[14] <= wl_bounds[1])]
        self.Iplus4 = self.Iplus4[:, (self.wl_scale[4] >= wl_bounds[0]) & (
            self.wl_scale[4] <= wl_bounds[1])]
        self.Iplus5 = self.Iplus5[:, (self.wl_scale[7] >= wl_bounds[0]) & (
            self.wl_scale[7] <= wl_bounds[1])]
        self.Iplus6 = self.Iplus6[:, (self.wl_scale[10] >= wl_bounds[0]) & (
            self.wl_scale[10] <= wl_bounds[1])]

        self.px_scale = np.array([self.px_scale[i][(self.wl_scale[i] >= wl_bounds[0]) & (
            self.wl_scale[i] <= wl_bounds[1])] for i in range(self.px_scale.shape[0])])
        self.wl_scale = np.array(
            [elt[(elt >= wl_bounds[0]) & (elt <= wl_bounds[1])] for elt in self.wl_scale])
    ...
    def spectralBinning(self, wl_min, wl_max, bandwidth, wl_to_px_coeff):
        ...
        self.p1 = self.p1[:, (self.wl_scale[15] >= wl_min)
                          & (self.wl_scale[15] <= wl_max)]
        self.p2 = self.p2[:, (self.wl_scale[13] >= wl_min)
                          & (self.wl_scale[13] <= wl_max)]
        self.p3 = self.p3[:, (self.wl_scale[2] >= wl_min)
                          & (self.wl_scale[2] <= wl_max)]
        self.p4 = self.p4[:, (self.wl_scale[0] >= wl_min)
                          & (self.wl_scale[0] <= wl_max)]

        self.Iminus1 = self.Iminus1[:, (self.wl_scale[11] >= wl_min) & (
            self.wl_scale[11] <= wl_max)]
        self.Iplus1 = self.Iplus1[:, (self.wl_scale[9] >= wl_min) & (
            self.wl_scale[9] <= wl_max)]
        self.Iminus2 = self.Iminus2[:, (self.wl_scale[3] >= wl_min) & (
            self.wl_scale[3] <= wl_max)]
        self.Iplus2 = self.Iplus2[:, (self.wl_scale[12] >= wl_min) & (
            self.wl_scale[12] <= wl_max)]
        self.Iminus3 = self.Iminus3[:, (self.wl_scale[1] >= wl_min) & (
            self.wl_scale[1] <= wl_max)]
        self.Iplus3 = self.Iplus3[:, (self.wl_scale[14] >= wl_min) & (
            self.wl_scale[14] <= wl_max)]
        self.Iminus4 = self.Iminus4[:, (self.wl_scale[6] >= wl_min) & (
            self.wl_scale[6] <= wl_max)]
        self.Iplus4 = self.Iplus4[:, (self.wl_scale[4] >= wl_min) & (
            self.wl_scale[4] <= wl_max)]
        self.Iminus5 = self.Iminus5[:, (self.wl_scale[5] >= wl_min) & (
            self.wl_scale[5] <= wl_max)]
        self.Iplus5 = self.Iplus5[:, (self.wl_scale[7] >= wl_min) & (
            self.wl_scale[7] <= wl_max)]
        self.Iminus6 = self.Iminus6[:, (self.wl_scale[8] >= wl_min) & (
            self.wl_scale[8] <= wl_max)]
        self.Iplus6 = self.Iplus6[:, (self.wl_scale[10] >= wl_min) & (
            self.wl_scale[10] <= wl_max)]

        self.px_scale_nonbinned = self.px_scale.copy()
        self.wl_scale_nonbinned = self.wl_scale.copy()

        self.px_scale = np.array([self.px_scale[i][(self.wl_scale[i] >= wl_min) & (
            self.wl_scale[i] <= wl_max)] for i in range(self.wl_scale.shape[0])])
        self.wl_scale = np.array(
            [elt[(elt >= wl_min) & (elt <= wl_max)] for elt in self.wl_scale])
        ...
        self.Iminus1 = self.binning(
            self.Iminus1, bandwith_px[11], axis=1, avg=True)
        self.Iminus2 = self.binning(
            self.Iminus2, bandwith_px[3], axis=1, avg=True)
        self.Iminus3 = self.binning(
            self.Iminus3, bandwith_px[1], axis=1, avg=True)
        self.Iminus4 = self.binning(
            self.Iminus4, bandwith_px[6], axis=1, avg=True)
        self.Iminus5 = self.binning(
            self.Iminus5, bandwith_px[5], axis=1, avg=True)
        self.Iminus6 = self.binning(
            self.Iminus6, bandwith_px[8], axis=1, avg=True)

        self.Iplus1 = self.binning(
            self.Iplus1, bandwith_px[9], axis=1, avg=True)
        self.Iplus2 = self.binning(
            self.Iplus2, bandwith_px[12], axis=1, avg=True)
        self.Iplus3 = self.binning(
            self.Iplus3, bandwith_px[14], axis=1, avg=True)
        self.Iplus4 = self.binning(
            self.Iplus4, bandwith_px[4], axis=1, avg=True)
        self.Iplus5 = self.binning(
            self.Iplus5, bandwith_px[7], axis=1, avg=True)
        self.Iplus6 = self.binning(
            self.Iplus6, bandwith_px[10], axis=1, avg=True)

        self.wl_scale = np.array([self.binning(
            self.wl_scale[i], bandwith_px[i], axis=0, avg=True) for i in range(self.wl_scale.shape[0])])
